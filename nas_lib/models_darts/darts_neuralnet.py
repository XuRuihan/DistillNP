...
class DartsADENeuralNet(nn.Module):
    def __init__(self, C, num_classes, layers, auxiliary, genotype, key, stem_mult=3):
        super().__init__()
        self.hashkey = key
        self._layers = layers
        ...
        self.classifier = nn.Conv2d(C_prev, num_classes, 1)

    def forward(self, input, device=None, drop_path_prob=0.0):
        logits_aux = None
        ...
        out = self.upsample(s1)
        logits = self.classifier(out)
        return logits, logits_aux


class DartsVOCNeuralNet(nn.Module):
    def __init__(self, C, num_classes, layers, auxiliary, genotype, key, stem_mult=3):
        super().__init__()
        self.hashkey = key
        self._layers = layers
